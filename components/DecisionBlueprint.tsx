...
export default function DecisionBlueprint({ data }: DecisionBlueprintProps) {
  // We use placeholder data to demonstrate the highly structured premium UI per your request.
  ...
  return (
    <motion.div 
      ...
      initial="hidden"
      animate="show"
      className="w-full max-w-5xl mt-12 md:mt-16 space-y-6 md:space-y-8 relative font-sans"
    >
      ...
      <motion.div variants={item} className="bg-neutral-900/40 backdrop-blur-md border border-white/10 rounded-3xl p-5 md:p-8 relative overflow-hidden group hover:bg-neutral-900/60 transition-colors">
        <h3 className="text-xl font-medium text-white mb-6 flex items-center"><Anchor className="w-5 h-5 mr-3 text-blue-400" /> Diagnosis</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          <div>
             <h4 className="text-xs uppercase text-neutral-500 tracking-wider mb-2 font-medium">Core Problem</h4>
             <p className="text-neutral-300 text-sm leading-relaxed">{data?.diagnosis?.coreProblem || 'Analyzing...'}</p>
          </div>
          <div>
             <h4 className="text-xs uppercase text-neutral-500 tracking-wider mb-2 font-medium">Blind Spots</h4>
             <p className="text-neutral-300 text-sm leading-relaxed">{data?.diagnosis?.blindSpots || 'Analyzing...'}</p>
          </div>
          <div>
             <h4 className="text-xs uppercase text-neutral-500 tracking-wider mb-2 font-medium">Key Risks</h4>
             <p className="text-neutral-300 text-sm leading-relaxed">{data?.diagnosis?.keyRisks || 'Analyzing...'}</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={item}>
        <h3 className="text-xl font-medium text-white mb-6 flex items-center ml-2"><Shuffle className="w-5 h-5 mr-3 text-purple-400" /> Decision Paths</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <div className="bg-neutral-900/30 backdrop-blur-md border border-emerald-500/10 rounded-3xl p-6 hover:border-emerald-500/30 transition-colors shadow-lg">
             <div className="flex items-center space-x-2 mb-4"><Shield className="w-5 h-5 text-emerald-400" /><h4 className="text-white font-medium">Safe Path</h4></div>
             <p className="text-neutral-400 text-sm mb-4">{data?.paths?.safe?.description || 'Details unavailable'}</p>
           ...
           <div className="bg-gradient-to-br from-neutral-900/50 to-neutral-800/50 backdrop-blur-md border border-purple-500/20 rounded-3xl p-6 hover:border-purple-500/40 transition-colors shadow-[0_0_40px_rgba(168,85,247,0.05)] relative overflow-hidden">
             ...
             <div className="flex items-center space-x-2 mb-4"><Zap className="w-5 h-5 text-purple-400" /><h4 className="text-white font-medium">Balanced Path</h4></div>
             <p className="text-neutral-300 text-sm mb-4 line-clamp-3">{data?.paths?.balanced?.description || 'Details unavailable'}</p>
           ...
           <div className="bg-neutral-900/30 backdrop-blur-md border border-rose-500/10 rounded-3xl p-6 hover:border-rose-500/30 transition-colors shadow-lg">
             <div className="flex items-center space-x-2 mb-4"><AlertOctagon className="w-5 h-5 text-rose-500" /><h4 className="text-white font-medium">Bold Path</h4></div>
             <p className="text-neutral-400 text-sm mb-4">{data?.paths?.bold?.description || 'Details unavailable'}</p>
             <ul className="text-xs text-neutral-500 space-y-2">
      ...
      <motion.div variants={item} className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 backdrop-blur-xl border border-blue-500/30 rounded-3xl p-5 md:p-8 relative overflow-hidden shadow-[0_0_60px_rgba(59,130,246,0.15)]">
        ...
        <h3 className="text-sm font-semibold uppercase tracking-widest text-blue-400 mb-3">Primary Insight & Recommendation</h3>
        <p className="text-xl text-white font-light leading-relaxed">
          {data?.recommendation || 'Recommendation pending...'}
        </p>
      </motion.div>
      ...
      <motion.div variants={item} className="bg-gradient-to-br from-orange-950/40 via-red-950/20 to-neutral-900/40 backdrop-blur-xl border border-orange-500/20 rounded-3xl p-5 md:p-8 relative overflow-hidden shadow-[0_0_60px_rgba(234,88,12,0.1)] group">
        ...
        <h3 className="text-xl font-medium text-white mb-6 flex items-center"><Flame className="w-5 h-5 mr-3 text-orange-500 group-hover:animate-pulse" /> Contrarian Insight</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        <motion.div variants={item} className="bg-neutral-900/40 backdrop-blur-md border border-white/10 rounded-3xl p-5 md:p-8 hover:bg-neutral-900/60 transition-colors">
           <h3 className="text-xl font-medium text-white mb-6 flex items-center"><TrendingUp className="w-5 h-5 mr-3 text-pink-400" /> Future Simulation</h3>
           <div className="space-y-6">
             <div className="flex space-x-4">
               ...
               <div>
                 <h4 className="text-sm text-neutral-300 font-medium mb-1">3 Months Out</h4>
             ...
             <div className="flex space-x-4">
               ...
               <div>
                 <h4 className="text-sm text-neutral-300 font-medium mb-1">12 Months Out</h4>
        ...
        {/* Card 5: Action Plan */}
        <motion.div variants={item} className="bg-neutral-900/40 backdrop-blur-md border border-white/10 rounded-3xl p-5 md:p-8 hover:bg-neutral-900/60 transition-colors">
           <h3 className="text-xl font-medium text-white mb-6 flex items-center"><CheckCircle className="w-5 h-5 mr-3 text-amber-400" /> Action Plan</h3>
           <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2.5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/10 before:to-transparent">
                 ...
                 <div className="flex items-center justify-center w-6 h-6 rounded-full border border-white/20 bg-neutral-900 text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-md">
                   <div className="flex items-center justify-between mb-1">
                     <div className="font-semibold text-white text-sm">Today</div>
                   </div>
                   <div className="text-neutral-400 text-xs text-wrap leading-relaxed">{data?.actionPlan?.today || 'TBD'}</div>
                 </div>
              </div>
              ...
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                 ...
                 <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-md">
                   <div className="flex items-center justify-between mb-1">
                     <div className="font-semibold text-white text-sm">This Week</div>
                   </div>
                   <div className="text-neutral-400 text-xs leading-relaxed">{data?.actionPlan?.thisWeek || 'TBD'}</div>
                 </div>
              </div>
              ...
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                 ...
                 <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl border border-white/5 bg-white/5 backdrop-blur-md">
                   <div className="flex items-center justify-between mb-1">
                     <div className="font-semibold text-white text-sm">30 Days</div>
                   </div>
                   <div className="text-neutral-400 text-xs leading-relaxed">{data?.actionPlan?.thirtyDays || 'TBD'}</div>
                 </div>
              </div>
           </div>
        </motion.div>
