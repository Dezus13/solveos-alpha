...
function OutcomeRecorder({
  ...
  const [scoreAccuracy, setScoreAccuracy] = useState(70);
  const [actualOutcome, setActualOutcome] = useState('');
  const [lesson, setLesson] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const [localDone, setLocalDone] = useState<Set<string>>(new Set());

  const visible = pending.filter(n => !localDone.has(n.id));
  ...
  const submit = async (id: string) => {
    if (!actualOutcome.trim() || submitting) return;
    setSubmitting(true);
    try {
      const res = await fetch('/api/outcomes', {
        ...
        body: JSON.stringify({
          ...
          outcome: {
            ...
            scoreAccuracy,
            lessons: lesson.trim() ? [lesson.trim()] : [],
            recommendations: [],
          },
        }),
      });
  ...
  return (
    <div className="rounded-2xl border border-amber-500/15 bg-amber-500/[0.03] p-6">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-1.5 h-1.5 rounded-full bg-amber-400 shadow-[0_0_8px_rgba(245,158,11,0.5)] animate-pulse" />
        <span className="text-[9px] font-black uppercase text-amber-300">
          {visible.length} Decision{visible.length !== 1 ? 's' : ''} Awaiting Outcome
        </span>
        <span className="ml-auto text-[8px] text-slate-600 font-mono uppercase">
          Each outcome strengthens calibration
        </span>
      </div>
      ...
      <div className="space-y-2">
        {visible.map(node => {
          ...
          const label = node.problem.length > 90
            ? node.problem.slice(0, 90) + '…'
            : node.problem;

          return (
            <div key={node.id} className="rounded-xl border border-white/10 bg-white/[0.015] overflow-hidden">
              <div className="flex items-center gap-3 px-4 py-3">
                <div className="flex-1 text-[11px] text-slate-400 leading-snug">{label}</div>
                <button
                  onClick={() => setActiveId(isActive ? null : node.id)}
                  className="flex-shrink-0 text-[9px] font-black uppercase px-3 py-1.5 rounded-lg border border-amber-500/25 text-amber-400 hover:bg-amber-500/10 transition-colors"
                >
                ...
                <div className="border-t border-white/5 px-4 pb-4 pt-4 space-y-4">
                  <div>
                    <div className="flex justify-between mb-2">
                      <span className="text-[8px] font-mono uppercase text-slate-500">AI Score Accuracy</span>
                      <span className="text-[8px] font-black font-mono text-slate-300">{scoreAccuracy} / 100</span>
                    </div>
                    <input
                      type="range"
                      min={0}
                    ...
                    <div className="flex justify-between mt-1">
                      <span className="text-[7px] text-slate-600">Completely wrong</span>
                      <span className="text-[7px] text-slate-600">Exactly right</span>
                    </div>
                  </div>

                  <div>
                    <div className="text-[8px] font-mono uppercase text-slate-500 mb-1.5">What actually happened</div>
                    <textarea
                      value={actualOutcome}
                      onChange={e => setActualOutcome(e.target.value)}
                      placeholder="Describe the real outcome…"
                      rows={2}
                      className="w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500/30 resize-none"
                    />
                  </div>

                  <div>
                    <div className="text-[8px] font-mono uppercase text-slate-500 mb-1.5">
                      Key lesson <span className="text-slate-600">(optional)</span>
                    </div>
                    <input
                      type="text"
                      value={lesson}
                      ...
                      className="w-full bg-white/[0.03] border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500/30"
                    />
                  </div>

                  <button
                    onClick={() => submit(node.id)}
                    disabled={!actualOutcome.trim() || submitting}
                    className="w-full py-2 rounded-lg bg-amber-500/10 border border-amber-500/25 text-amber-300 text-[10px] font-black uppercase hover:bg-amber-500/20 transition-colors disabled:opacity-40"
                  >
